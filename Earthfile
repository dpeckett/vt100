...
package:
  ...
  # Build Dependencies
  RUN apt install -y golang-github-stretchr-testify-dev
  RUN mkdir -p /workspace/golang-github-jaguilar-vt100
  WORKDIR /workspace/golang-github-jaguilar-vt100
  COPY . .
  ...
  RUN export DEBEMAIL="damian@pecke.tt" \
    ...
    && dch --create --package golang-github-jaguilar-vt100 --newversion "${VERSION}-1" \
      --distribution "UNRELEASED" --force-distribution  --controlmaint "Last Commit: $(git log -1 --pretty=format:'(%ai) %H %cn <%ce>')" \
    && tar -czf ../golang-github-jaguilar-vt100_${VERSION}.orig.tar.gz .
